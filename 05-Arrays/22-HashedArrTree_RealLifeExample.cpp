...
private:
    std::vector<std::vector<T>> top; // Top-level array (stores leaf arrays)
    ...
    void expand() {
        ...
        std::vector<std::vector<T>> new_top(new_top_size);
    ...
    void reallocate_leaf(size_t top_idx) {
        std::vector<T> new_leaf(leaf_size);
...
public:
    ...
    HashedArrayTree(size_t initial_leaf_size = 1)
        ...
        top.push_back(std::vector<T>(leaf_size));
    }
    ...
    HashedArrayTree(HashedArrayTree&& other) noexcept
        ...
        other.size_ = 0;
        other.capacity = 1;
        other.top = std::vector<std::vector<T>>(1);
    }
    ...
    // Destructor (handled automatically by vector)
    ...
    void push_back(const T& value) {
        ...
        if (top[top_idx].empty()) {
            top[top_idx] = std::vector<T>(leaf_size);
        }
...
int main() {
    HashedArrayTree<Student> studentRecords;

    // Insert student records
    studentRecords.push_back(Student("Alice", 1, 3.8));
    studentRecords.push_back(Student("Bob", 2, 3.5));
    studentRecords.push_back(Student("Charlie", 3, 3.9));
    studentRecords.push_back(Student("David", 4, 3.7));

    // Access and print student records
    ...
    HashedArrayTree<Student> studentRecordsCopy = studentRecords;
    std::cout << "\nCopied Student Records:" << std::endl;
    for (size_t i = 0; i < studentRecordsCopy.size(); ++i) {
        studentRecordsCopy[i].print();
    }

    // Test move constructor
    HashedArrayTree<Student> studentRecordsMoved = std::move(studentRecordsCopy);
    std::cout << "\nMoved Student Records:" << std::endl;
    for (size_t i = 0; i < studentRecordsMoved.size(); ++i) {
        studentRecordsMoved[i].print();
    }
